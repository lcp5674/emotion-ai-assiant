...
def test_token_storage_fix():
    """测试Token存储安全优化"""
    print("=" * 60)
    print("测试4: Token存储安全优化")
    print("=" * 60)
    
    frontend_path = os.path.dirname(__file__)
    store_path = os.path.join(frontend_path, 'frontend/web/src/stores/index.ts')
    ...
    # 检查是否添加了安全注释
    checks = [
        ('安全优化说明' in content, '添加了安全优化说明注释'),
        ('try-catch' in content, '添加了try-catch错误处理'),
        ('httpOnly' in content, '提到了httpOnly Cookie方案'),
        ('localStorage' in content, '保留了localStorage（兼容性）'),
    ]
...
def test_websocket_url_fix():
    """测试WebSocket URL配置优化"""
    print("=" * 60)
    print("测试5: WebSocket URL配置优化")
    print("=" * 60)
    
    frontend_path = os.path.dirname(__file__)
    request_path = os.path.join(frontend_path, 'frontend/web/src/api/request.ts')
    ...
    # 检查是否优化了WebSocket URL配置
    checks = [
        ('VITE_WS_BASE_URL' in content, '使用环境变量VITE_WS_BASE_URL'),
        ('window.location.protocol' in content, '智能检测协议（http/https）'),
        ('wss:' in content, '支持wss:协议'),
        ('ws:' in content, '支持ws:协议'),
    ]
    ...
    # 检查是否还硬编码了 localhost:8000
    if 'ws://localhost:8000' in content and 'import.meta.env.VITE_WS_BASE_URL ||' not in content:
        print("  ✗ 仍存在硬编码的 ws://localhost:8000")
        all_passed = False
    
    if all_passed:
        print("\n✅ WebSocket URL配置优化已应用！\n")
    else:
        print("\n❌ WebSocket URL配置优化有问题！\n")
...
def test_transaction_fix():
    ...
    backend_path = os.path.join(os.path.dirname(__file__), 'backend')
    chat_service_path = os.path.join(backend_path, 'app/services/chat_service.py')
    
    print(f"\n检查文件: {chat_service_path}")
    ...
    # 检查是否优化了事务处理
    checks = [
        ('transaction_started = False' in content, '添加了事务状态跟踪'),
        ('db.begin()' in content, '显式事务管理'),
        ('db.rollback()' in content, '事务回滚处理'),
        ('_send_message_logic' in content, '业务逻辑分离'),
        ('优化说明' in content, '添加了优化说明注释'),
    ]
...
def main():
    ...
    # 中优先级问题
    results.append(("Token存储安全优化", test_token_storage_fix()))
    results.append(("WebSocket URL配置优化", test_websocket_url_fix()))
    results.append(("事务处理优化", test_transaction_fix()))
